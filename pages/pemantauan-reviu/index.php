...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update'])) {
    ...
    if (isset($_FILES['document']) && $_FILES['document']['error'] === UPLOAD_ERR_OK) {
        ...
        if (array_key_exists($mimeType, $allowedTypes) && $_FILES['document']['size'] <= $maxSize) {
            ...
            if (move_uploaded_file($_FILES['document']['tmp_name'], $destination)) {
                ...
                $data['document_path'] = $documentPath; // tambahkan ke data update
            } else {
        ...
        $_SESSION['toast_message'] = [
            "icon" => "error",
            "message" => "Error upload: " . $_FILES['document']['error']
        ];
        echo '<script>window.history.back();</script>';
        exit;
    }
    ...
    echo '<script>window.location.href = "' . URLEnum::getMonitoringReviewURL() . '";</script>';
}

if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['verify'])) {
    $id         = isset($_POST['monitoring_id']) ? $_POST['monitoring_id'] : '';
    $is_verified = isset($_POST['is_verified']) ? $_POST['is_verified'] : '';
    $notes      = isset($_POST['notes']) ? $_POST['notes'] : '';

    if ($monitoringReviewModel->verifyMonitoringReview($id, $is_verified, $notes)) {
        $toastMessage = array(
            "icon" => "success",
    ...
    $url = URLEnum::getMonitoringReviewURL();
    if (isset($_GET['act'])) {
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    if (isset($_GET['act'])) {
        ...
        if ($action == "delete") {
            $id     = $_GET['id'];
            if ($reviewModel->deleteRiskAssessment($id)) {
                $toastMessage = array(
                    "icon" => "success",
            ...
            echo '<script>window.location.href = "' .  URLEnum::getMonitoringReviewURL() . '?act=show' . '&unit_id=' . $unit_id . '&category=' . $riskCategorySelected . '";</script>';
        }
                    ...
                    <div class="card">
                        ...
                        <div class="card-body table-responsive">
                            <table id="monitoring_reviu-table" class="table table-bordered table-striped risk-assessment-table">
                                ...
                                <tbody>
                                    <?php
                                    if (isset($allMonitoringReviews) && count($allMonitoringReviews) > 0) {
                                        $i = 0;

                                        foreach ($allMonitoringReviews as $review) {
                                            $i++;
                                            $date = new DateTime($review['updated_at']);
                                            $updated_at = $date->format('d M Y H:i'); ?>

                                            <tr>
                                                <td class="text-center"><?= $i ?></td>
                                                <td><?= $review['risiko'] ?></td>
                                                <td><?= $review['kategori_risiko'] ?></td>
                                                <td><?= $review['p_analisis'] ?></td>
                                                <td><?= $review['d_analisis'] ?></td>
                                                <td><?= $review['bobot_analisis'] ?></td>
                                                <td><?= $review['nilai_analisis'] ?></td>
                                                <td><?= $review['tingkat_risiko_analisis'] ?></td>
                                                <td><?= $review['prioritas_risiko'] ?></td>
                                                <td><?= $review['uraian_pengendalian'] ?></td>
                                                <td><?= $review['jadwal_pelaksanaan'] ?></td>
                                                <td><?= $review['p'] ?></td>
                                                <td><?= $review['d'] ?></td>
                                                <td><?= $review['bobot'] ?></td>
                                                <td><?= $review['nilai'] ?></td>
                                                <td><?= $review['tingkat_risiko_pemantauan'] ?></td>
                                                <td><?= $review['tingkat_risiko_simpulan'] ?></td>
                                                <td><?= $review['efektif'] ?></td>
                                                <td class="text-center"><?= $updated_at ?></td>
                                                <td class="text-center">
                                                    <?php
                                                    if ($review['is_verified'] === 1 && $review['is_verified'] !== null) {
                                                        $date = new DateTime($review['verified_at']);
                                                    ...
                                                    } elseif ($review['is_verified'] === 0 && $review['is_verified'] !== null) {
                                                        $date = new DateTime($review['verified_at']);
                                                        $verified_at = $date->format('d/m/Y H:i');
                                                ...
                                                <td class="text-center"><?= $review['notes'] ?></td>
                                                ...
                                                if ($_SESSION['role'] == RoleEnum::ADMIN) { ?>
                                                    <td class="text-center">
                                                        <div class="btn-group">
                                                            <?php
                                                            if ($review['is_verified'] === null) :
                                                                $disabled = ($review['p'] === null || $review['d'] === null) ? 'disabled' : '';
                                                                $notes = htmlspecialchars($review['notes'] ?? '', ENT_QUOTES);
                                                            ?>
                                                                <button
                                                                    class="btn btn-sm btn-success modal-verif-pemantauan"
                                                                    data-toggle="modal"
                                                                    data-target="#modal-verif-pemantauan"
                                                                    data-id="<?= $review['id'] ?>"
                                                                    data-notes="<?= $notes ?>"
                                                                    <?= $disabled ?>>
                                                            ...
                                                            <?php endif ?>
                                                            <a href="<?= URLEnum::getMonitoringReviewURL() . '?act=delete' . '&id=' . $review['id'] ?>" class="btn btn-danger">
                                                                <i class="fas fa-trash"></i>
                                                            </a>
                                                        </div>
                                                    </td>
                                                <?php
                                                } elseif ($_SESSION['role'] == RoleEnum::UNIT && $user['unit_id'] == $review['unit_id']) {
                                                    $isDisabled = $review['is_verified'] == 1 ? 'disabled' : '';
                                                ?>
                                                    <td class="text-center">
                                                        <div class="btn-group">
                                                            <button type="button"
                                                                ...
                                                                data-id="<?= $review['id'] ?>"
                                                                data-nilai-analisis="<?= $review['nilai_analisis'] ?>"
                                                                data-p="<?= $review['p'] ?>"
                                                                data-d="<?= $review['d'] ?>"
                                                                <?= $isDisabled ?>>
                                                                <i class="fas fa-edit"></i>
                                        ...
                                        echo '
                                        <tr>
                                            <td colspan="28" class="text-center">Tidak ada ' . $pageName . '</td>
                                        </tr>
                                    ';
                                    }
                                        ?>
...
<!-- Modal Verifikasi Penilaian Risiko -->
<div class="modal fade" id="modal-verif-pemantauan">
    <div class="modal-dialog">
        <div class="modal-content">
            <form method="POST" action="">
                <!-- Hidden risk_id -->
                <input type="hidden" class="form-control" name="monitoring_id" id="modal-pemantauan-id">
                ...
                <div class="modal-body">
                    ...
                    <div id="modal-verif-status" class="text-center mt-3">
                        <!-- Akan diisi lewat JS -->
                    </div>

                    <div class="form-group mt-4">
                        <label class="d-block text-center mt-4">Verifikasi Penilaian Risiko<span class="text-danger">*</span></label>
                        <div class="row justify-content-center">
                            <div class="icheck-success col-md-2">
                                <input class="form-check-input" type="radio" name="is_verified" value="1"
                                    id="modal-verified-yes" onchange="onShowNotes(false)">
                                <label class="form-check-label" for="modal-verified-yes">Ya<span class="text-danger">*</span></label>
                            </div>
                            <div class="icheck-danger col-md-2">
                                <input class="form-check-input" type="radio" name="is_verified" value="0"
                                    id="modal-verified-no" onchange="onShowNotes(true)">
                                <label class="form-check-label" for="modal-verified-no">Tidak<span class="text-danger">*</span></label>
                            </div>
                        </div>
                    </div>

                    <div class="form-group mt-2" id="modal-notes-wrapper" style="display: none;">
                        <h6 class="d-block">Catatan<span class="text-danger">*</span></h6>
                        <textarea id="modal-notes" class="form-control" name="notes"></textarea>
                    </div>
                </div>
...
<script>
    ...
    function onChangeFilter(unitId, categorySelected, monthSelected, yearSelected) {
        ...
        window.location.href = "<?= URLEnum::getMonitoringReviewURL() ?>?" + queryString;
    }
